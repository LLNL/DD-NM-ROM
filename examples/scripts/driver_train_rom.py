...
net_folder0 = args.save_dir + f'/trained_nets/{name}/multi/'
fig_folder0 = args.save_dir + f'/figures/loss_hist/{name}/multi/'
...
for phase in phases:
  for sub in sub_list:
    ...
    snapshots = torch.from_numpy(data[phase][sub]).type(torch.float32)

    state     = [uv_dd[phase]["u"][sub], uv_dd[phase]["v"][sub]]
