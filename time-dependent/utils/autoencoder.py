...
def RelMSELoss(outputs, pred, epsilon=0.0):
    '''
    ...
    '''
    return torch.mean(torch.sum(torch.square(outputs-pred), dim=1)/(torch.sum(torch.square(outputs), dim=1)+epsilon))
...
class Autoencoder:
    ...
    def __init__(self, snapshots, latent_dim, row_nnz, row_shift, device, 
        ...
        snapshots = snapshots.to(self.device)
        ...
        if normalize:
            ...
            self.ref = np.zeros(snapshots.shape[1])
            self.scale = np.ones(snapshots.shape[1])
        
        self.train_data = TensorDataset((snapshots[train_ind]-self.ref)/self.scale)
        self.test_data = TensorDataset((snapshots[test_ind]-self.ref)/self.scale)
...
#                                self.ref,
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, patience=self.lr_patience, min_lr=min_lr)
...
#                 outputs = data.float().to(self.device)
...
#                 outputs = data.float().to(self.device)
            ...
            if epoch % n_epochs_print == 0:
                print(f'Train {self.loss_type} Loss: {epoch_train_loss:1.4e}')
                print(f' Test {self.loss_type} Loss: {epoch_test_loss:1.4e}')
                sys.stdout.flush()
        ...
        print('-' * 10)
        print(f'Final Train {self.loss_type} Loss: {train_loss_hist[-1]:1.4e}')
        print(f' Final Test {self.loss_type} Loss: {test_loss_hist[-1]:1.4e}')
        sys.stdout.flush()
